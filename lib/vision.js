 ...
 *
 * `--ffmpeg-location` NO es opcional: bajar video+audio por separado obliga a yt-dlp a
 * MULTIPLEXARLOS, y para eso necesita ffmpeg. Aca ffmpeg vino anidado adentro del paquete
 * WinGet de yt-dlp y no esta en el PATH, asi que yt-dlp no lo encuentra solo: bajaba los
 * dos streams, no podia unirlos, y dejaba `v.fdash-...a.m4a` + `v.fdash-...v.mp4` sueltos.
 * Ninguno matcheaba el patron y la capa visual reportaba "no se pudo bajar el video"
 * — con los dos archivos ahi al lado.
 */
function bajar(url, tmp) {
  const ff = bin('ffmpeg');
  const args = ['-f', FORMATO, '--no-warnings', '-o', path.join(tmp, 'v.%(ext)s')];
  if (ff) args.push('--ffmpeg-location', path.dirname(ff));
  try {
    execFileSync(bin('yt-dlp'), [...args, url], { stdio: 'ignore', timeout: 600000 });
  } catch { /* puede fallar un formato y haber bajado otro: manda el disco */ }
  // Se acepta cualquier `v.*` con pista de video, no solo el nombre exacto: si el merge
  // igual no ocurrio, es mejor analizar el stream de video suelto que declarar falla total.
  const cand = fs.readdirSync(tmp).filter(x => /^v.*\.(mp4|webm|mkv)$/i.test(x))
    .map(x => path.join(tmp, x));
  return cand.find(c => medios.medir(c).video) ?? null;
}
